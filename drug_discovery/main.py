...
def main(config):
    ...
    for i in range(start_iters, self.num_iters):
        ...
        if config.quantum:
            sample_list = [torch.tensor(gen_circuit(gen_weights)).to(self.device).float() for i in range(self.batch_size)]
            print(f"sample_list: {len(sample_list)}: {len(sample_list[0])}")
